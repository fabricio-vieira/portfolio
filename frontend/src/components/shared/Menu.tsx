'use client'
import { usePathname } from 'next/navigation'
import Link from 'next/link'

export default function Menu() {
    const caminhoAtual = usePathname()

    return (
        <nav className="flex items-center gap-6">
            <MenuItem href="/" selecionado={caminhoAtual === '/'}>
                {' '}
                Inicio
            </MenuItem>
            <MenuItem href="/projeto/1" selecionado={caminhoAtual.startsWith('/projeto')}>
                {' '}
                Projeto
            </MenuItem>
            <MenuItem href="/sobre" selecionado={caminhoAtual === '/sobre'}>
                {' '}
                Sobre
            </MenuItem>
            <MenuItem href="https://api.whatsapp.com/send?phone=5511999999999" selecionado={false} novaAba={true}>
                {' '}
                Contato
            </MenuItem>
        </nav>
    )
}

function MenuItem(props: { href: string; children: React.ReactNode; selecionado: boolean; novaAba?: boolean }) {
    return (
        <Link href={props.href} target={props.novaAba ? '_blank' : '_self'}>
            <span
                className={`
                flex items-center gap-2 text-sm border-red-600 hover:text-white
                ${props.selecionado ? 'border-b-4 text-white' : 'text-zinc-400'}
            `}
            >
                {' '}
                {props.children}
            </span>
        </Link>
    )
}
